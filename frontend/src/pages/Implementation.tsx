          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <Callout variant="note" title={es ? 'El gate decide el lane' : 'The gate decides the lane'}>
                {es ? 'Un caso se ejecuta en la lane live si es client-side, sus runtimes ⊆ {ts-mincut, onnxruntime-web} y el solve + la traza caben en presupuesto. Dinic es el rung interactivo; pseudoflow es el rung exacto de validación comparativa.' : 'A case runs in the live lane if it is client-side, its runtimes ⊆ {ts-mincut, onnxruntime-web} and the solve + trace fit budget. Dinic is the interactive rung; pseudoflow is the independent exact comparison rung.'}
              </Callout>
            </div>
          ),
        },
...
cd frontend && npm test                 # 50 contracts (47 pass + 3 cache-gated)
npm run test:e2e                        # 14 real-browser UI contracts
