...
|---|---|---|
| **three.js** (`viz/PitView3D.tsx`) | the App’s 3-D tab | the orebody as voxels (InstancedMesh, one draw call for ~7 000 blocks), coloured by grade (viridis); the extracted pit / orebody / pushback modes; orbit controls; z increases downward so the pit opens from the surface. Lazy-chunked off the main bundle. |
