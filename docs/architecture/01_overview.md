...
is TypeScript that runs in the browser, so the App re-solves the pit exactly as the sliders move. There is no
...
`block model (synthetic or user-supplied)` → **[Contract 1](08_data-contracts.md)** (`io/contract.py`) → the TS optimiser
